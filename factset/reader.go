...
func NewReader(config sftpConfig) (FReader, error) {
...
func (sfr *Reader) Close() {
	if sfr.client != nil {
		sfr.client.Close()
	}
}

func (sfr *Reader) Read(fRes factsetResource, dest string, isWeekly bool) ([]zipCollection, error) {
...
func (sfr *Reader) download(filePath string, fileName string, dest string) error {
	start := time.Now()
...
func (sfr *Reader) GetMostRecentZips(files []os.FileInfo, searchedFileName string) ([]string, error) {
	foundFile := &struct {
		name         string
		majorVersion int
		minorVersion int
...
func (sfr *Reader) unzip(archive string, factsetFiles []string, dest string) ([]string, error) {
...
func (sfr *Reader) getMajorVersion(fullVersion string) (int, error) {
...
func (sfr *Reader) getMinorVersion(fullVersion string) (int, error) {
