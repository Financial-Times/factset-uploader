...
func (c *Client) DropTablesWithProductAndBundle(product string, bundle string) error {
	...
	if err != nil {
		log.WithError(err).WithFields(log.Fields{"fs_product": product}).Errorf("Error running query to return tables matching: %s/%s", product, bundle)
		return err
	}
	...
	for rows.Next() {
		...
		if err != nil {
			log.WithError(err).WithFields(log.Fields{"fs_product": product}).Errorf("Error scanning rows for tables matching: %s/%s", product, bundle)
			return err
		}
	...
	if len(tableNames) == 0 {
		log.WithFields(log.Fields{"fs_product": product}).Infof("Db has no tables matching: %s/%s", product, bundle)
		return nil
	}
...
func (c *Client) UpdateLoadedTableVersion(tableName string, version factset.PackageVersion, product string, bundle string) error {
	...
	res, err := stmt.Exec(tableName, version.FeedVersion, version.Sequence, product, bundle)
	if err != nil {
...
func (c *Client) CreateTablesFromSchema(contents []byte, product string, bundle string) error {
	...
	for _, statement := range statements {
		...
		if statement != "" && len(statement) > 10 {
			...
			if statementSplits[0] == "CREATE" && statementSplits[1] == "TABLE" {
				if err = c.UpdateLoadedTableVersion(statementSplits[2], factset.PackageVersion{0, 0}, product, bundle); err != nil {
					return err
				}
			}
		}
